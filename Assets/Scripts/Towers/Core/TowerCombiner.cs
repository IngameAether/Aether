...
namespace Towers.Core
{
    ...
    public class TowerCombiner : MonoBehaviour
    {
        ...
        {
            Tile clickedTile = clickedElementController.parentTile;
            ...
            {
                ElementType firstSelectedType = _selectedTiles[0].CurrentLogicalElementType;
                ElementType clickedType = clickedTile.CurrentLogicalElementType;
        ...
        {
            ElementType typeOfTowerToBuild = _selectedTiles[2].CurrentLogicalElementType;
            if (_elementTowerMap.TryGetValue(typeOfTowerToBuild, out var towerPrefab))
            {
                if (towerPrefab)
                {
                    foreach (Tile selectedTile in _selectedTiles)
                    {
                        ...
                        TileInteraction tileInt = selectedTile.GetComponent<TileInteraction>();
                        if(tileInt != null)
                        {
                    ...
                    Tile targetTile = _selectedTiles[2]; // 세 번째 타일에서 타워 생성
                    TileInteraction targetTileInteraction = targetTile.GetComponent<TileInteraction>();

                    if(targetTileInteraction != null)
                    {
                        GameObject newTower = targetTileInteraction.PlacedTower(towerPrefab, targetTile);
